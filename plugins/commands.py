...
async def clean_db_command(client, message):
    ...
    try:
        ...
        summary.append(f"🗃 DB1 — Matched: <code>{result1.matched_count}</code>, Modified: <code>{result1.modified_count}</code>")
        ...
        if MULTIPLE_DB:
            ...
            summary.append(f"🗃 DB2 — Matched: <code>{result2.matched_count}</code>, Modified: <code>{result2.modified_count}</code>")
        else:
